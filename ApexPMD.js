    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        for (let i = 0; i < partSubArray.length; i++) {
          let bodyPost = {
            opType: "ATTACHMENT",
            attachment: JSON.stringify(partSubArray[i]),
          }; //,"00P5g000000y28QEAQ"
          self.connSourceOrg.apex.post(
            URL_POST,
            bodyPost,
            function (err, result) {
              if (err) {
              ...
              for (var prop in mapBody) {
                ...
                zipEntries.forEach(function (zipEntry) {
                  if (
                    ...
                    (zipEntry.name.endsWith(".trigger") ||
                      zipEntry.name.endsWith(".page") ||
                      zipEntry.name.endsWith(".cls"))
                  ) {
                    fs.writeFileSync(
                      tmpFolder + zipEntry.name,
          ...
          self.connSourceOrg.apex.post(
            ...
            function (err, result) {
              ...
              if (text == "Null attachment") {
                ...
                self.createErrorLog(
                  "Error getting rules: attachment not found."
                );
                reject("error");
              } else {
                var tmpFolder = "./" + self.jobId + "/";
          ...
          if (fs.existsSync("./" + self.jobId + "/ruls.xml")) {
            var workerProcess = child_process.execSync(
              "bash dist/pmd-bin/bin/pmd check --no-fail-on-violation --dir ./" +
                ...
                "/result.csv --rulesets ./" +
                self.jobId +
                "/ruls.xml --property problem=false --property package=false --property ruleSet=false -z " +
                path.resolve("./" + self.jobId) +
                "/ -t 0",
        ...
        if (fs.existsSync("./" + self.jobId + "/result.csv")) {
          ...
          self.connSourceOrg.sobject("Attachment").create(
            ...
            function (err, ret) {
              ...
              self.attId = ret.id;
              self.connSourceOrg.query(
                "SELECT Id, Name FROM " + NAME_SPACE_PREFIX + "Rule__c",
                function (err, result) {
                  if (err) {
                    self.isContinue = false;
                    self.createErrorLog(err);
  ...
  updateObjects() {
    return new Promise((resolve, reject) => {
      try {
        ...
        if (!self.isContinue) {
          self.connSourceOrg
            .sobject(NAME_SPACE_PREFIX + "Flosum_Task__c")
            .update(
              {
                ["Id"]: self.jobId,
        ...
        self.connSourceOrg
          .sobject(NAME_SPACE_PREFIX + "Review_Result__c")
          .find({
          ...
          .update(
            ...
            function (err, rets) {
              ...
              for (let i = 0; i < self.violList.length; i++) {
                reviewViolationList.push({
                  [NAME_SPACE_PREFIX + "File_Name__c"]: self.violList[i].name,
                  [NAME_SPACE_PREFIX + "Priority__c"]: self.violList[i].prior,
                  [NAME_SPACE_PREFIX + "Position__c"]: self.violList[i].pos,
                  [NAME_SPACE_PREFIX + "Rule__c"]:
                    self.mapRuls[self.violList[i].rule],
                  [NAME_SPACE_PREFIX + "Error_Description__c"]:
                    self.violList[i].desc,
              ...
              self.connSourceOrg
                .sobject(NAME_SPACE_PREFIX + "Review_Violation__c")
                .create(
                  reviewViolationList,
                  { allowRecursive: true },
                  function (err, rets) {
    ...
    self.connSourceOrg.sobject(NAME_SPACE_PREFIX + "Flosum_Task__c").update(
      {
        ...
        [NAME_SPACE_PREFIX + "State__c"]: "CANCELED",
        [NAME_SPACE_PREFIX + "Comment__c"]:
          'Heroku service error. See Attachment "ApexPMD error" for details.',
      },
      function (err, ret) {
        if (err || !ret.success) {
          return console.log(err, ret);
